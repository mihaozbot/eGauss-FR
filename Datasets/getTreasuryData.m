function [data, trainData, testData, m, params, name] = getTreasuryData(n_C, p_gate, doNormalize, testFraction)
%https://sci2s.ugr.es/keel/dataset.php?cod=42#sub2
...
fprintf("Loading %s …\n", datFile);
if ~isfile(datFile), error('File "%s" not found.', datFile); end
...
for k = 1:n_C
    ...
    Xk = X_train(:, startI:stopI);
    yk = y_train(:, startI:stopI);
    trainData{k} = [Xk; yk];  % (m+1)×rows
